...
/*    std::cout << std::endl << std::endl << std::endl;
    ...
    for (int i = 0; i < this->net()->layers().size(); ++i) {
      for (int j = 0; j < this->net()->layers()[i]->blobs().size(); ++j) {
        Blob<Dtype>& blob = *this->net()->layers()[i]->blobs()[j];
        ...
        std::cout << "    [Ze] -0" << world_rank 
          << "- Layer " << this->net()->layer_names()[i] << " data: " << data_abs_val_mean
          << std::endl;
      }
    }*/

    net_->Update();
//////Broadcast the parameters inside of each layer after the network update//////
    //std::cout << "After the Update() for iteration " << iter_ << std::endl;
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  //after the initialization, trying to synchornize the data do every node.
    /*std::cout << "Total number of layers is " << this->net()->layers().size() << std::endl;
    for (int i = 0; i < this->net()->layers().size(); ++i) {
      for (int j = 0; j < this->net()->layers()[i]->blobs().size(); ++j) {
        Blob<Dtype>& blob = *this->net()->layers()[i]->blobs()[j];
        Dtype data_abs_val_mean = blob.asum_data() / blob.count();
  ...
  //std::cout << "max_iter is " << param_.max_iter() << ", iter_ is " << iter_ << std::endl;
