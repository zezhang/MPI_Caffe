...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  /*std::cout << std::endl << std::endl << "Before the Reshape:" << std::endl;
  std::cout <<  "kernel_h_ " << kernel_h_ << "     kernel_w_ " << kernel_w_ << std::endl;
  ...
  std::cout <<  "pad_h_ " << pad_h_ << "     pad_w_ " << pad_w_ << std::endl;
  std::cout <<  "height_ " << height_ << "     width_ " << width_ << std::endl;
  std::cout << "group_ " << group_ << std::endl;
  std::cout << "num_output_ " << num_output_ << std::endl;
  std::cout <<  "height_out_ " << height_out_ << "     width_out_ " << width_out_ << std::endl;
  std::cout << "has bias_term_ " << bias_term_ << std::endl;
  std::cout << "is_1x1_ " << is_1x1_ << std::endl;
  std::cout << "conv_out_channels_ " << conv_out_channels_ << std::endl;
  std::cout << "conv_in_channels_ " << conv_in_channels_ << std::endl;
  std::cout << "conv_out_spatial_dim_ " << conv_out_spatial_dim_ << std::endl;
  std::cout << "conv_in_height_ " << conv_in_height_ << std::endl;
  std::cout << "conv_in_width_ " << conv_in_width_ << std::endl;
  ...
  /*std::cout << std::endl << "After the Reshape:" << std::endl;
  std::cout <<  "kernel_h_ " << kernel_h_ << "     kernel_w_ " << kernel_w_ << std::endl;
  std::cout <<  "stride_h_ " << stride_h_ << "     stride_w_ " << stride_w_ << std::endl;
  std::cout << "num_ " << num_ << std::endl;
  std::cout << "channels_ " << channels_ << std::endl;
...
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
  ...
  if (!is_1x1_) {
    if (!skip_im2col) {
      //std::cout << "Inside of the loop!!!" << std::endl;
      conv_im2col_cpu(input, col_buffer_.mutable_cpu_data());
    }
    col_buff = col_buffer_.cpu_data();
  }
  //std::cout << "group_ is " << group_ << std::endl;
...
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  //std::cout << "sum of col_buffer_ is " << col_buffer_.asum_data() << std::endl;
  if (is_1x1_) {
    //std::cout << "check point 1" << std::endl;
    //doesn't goes in here
    col_buff = input;
  }
  for (int g = 0; g < group_; ++g) {
    //LOG(INFO) << "[Ze]:---------------kernel_dim_ / group_ (M) is " << kernel_dim_ / group_;
    //LOG(INFO) << "[Ze]:---------------conv_out_spatial_dim_ (N) is " << conv_out_spatial_dim_;
    //LOG(INFO) << "[Ze]:---------------conv_out_channels_ / group_ (K) is " << conv_out_channels_ / group_;
...
void BaseConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
  //LOG(INFO) << "[1]:---------Before------bias_multiplier is " << bias_multiplier_.asum_data() / bias_multiplier_.count();
  //std::cout << "M is " << num_output_ << std::endl;
  //std::cout << "N is " << height_out_ * width_out_ << std::endl;
  //std::cout << "bias_multiplier_ is " << bias_multiplier_.shape_string() << std::endl;
